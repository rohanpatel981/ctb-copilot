...
class TenantScope(BaseModel):
    ...
    def as_filter_pairs(self) -> list[tuple[str, str]]:
        """Snake-case (column, value) pairs for every field that's set.
        Used by the query path to inject mandatory tenant filters into
        whatever SQL Claude generates."""
        ...
        if self.fin_year_id is not None:
            pairs.append(("fin_year_id", self.fin_year_id))
        if self.reporting_period_id is not None:
            pairs.append(("reporting_period_id", self.reporting_period_id))
        return pairs
